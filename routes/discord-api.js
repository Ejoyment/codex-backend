 ...
 *     responses:
 *       200:
 *         description: Returns Discord authorization URL
 *       401:
 ...
 *     responses:
 *       200:
 *         description: Discord connected successfully
 *       400:
...
router.get('/callback', async (req, res) => {
    try {
        ...
        // Fetch Discord user info to store alongside the integration
        const discordUser = await discordAPI(access_token, '/users/@me');
        ...
        await Integration.findOneAndUpdate(
            { userId, provider: 'discord' },
            {
                ...
                isActive: true,
                accountUsername: discordUser.username,
 ...
 * /api/discord/status:
 *   get:
 *     summary: Check Discord connection status
 *     tags:
 *       - Discord API
 *     security:
 *       - bearerAuth: []
...
router.get('/status', verifyToken, async (req, res) => {
    try {
        ...
        res.json({
            ...
            account: integration ? integration.accountUsername : null
        });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
