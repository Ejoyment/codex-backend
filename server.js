  ...
  return new Promise((resolve, reject) => {
    srv
      .listen(port, () => {
        ...
        console.log(
          `💳 Trial Billing: Active (210s first charge, 2 month second charge)`,
        );
...
const stopServer = async () => {
  try {
    ...
    if (collaboration && collaboration.stopPersistenceWorker)
      collaboration.stopPersistenceWorker();

    const terminalService = require("./utils/terminalService");
    if (terminalService && terminalService.stopCleanup)
      terminalService.stopCleanup();
